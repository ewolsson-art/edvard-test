...
const Characteristics = () => {
  ...
  const latestMood = entries.length > 0 ? entries.sort((a, b) => b.timestamp - a.timestamp)[0]?.mood : null;

  const moodCards = [
    { type: 'elevated' as const, slug: 'uppvarvad', title: t('characteristics.elevated'), subtitle: t('characteristics.elevatedSub'), icon: Zap, iconColor: 'text-amber-400', dotColor: 'bg-amber-400', chipColor: 'text-amber-300/70' },
    { type: 'stable' as const, slug: 'stabil', title: t('characteristics.stable'), subtitle: t('characteristics.stableSub'), icon: Sun, iconColor: 'text-emerald-400', dotColor: 'bg-emerald-400', chipColor: 'text-emerald-300/70' },
    { type: 'depressed' as const, slug: 'nedstamd', title: t('characteristics.depressed'), subtitle: t('characteristics.depressedSub'), icon: Cloud, iconColor: 'text-rose-400', dotColor: 'bg-rose-400', chipColor: 'text-rose-300/70' },
  ];
      ...
      <div className="max-w-2xl mx-auto md:mx-0 p-5 md:p-8">
        ...
        <div className="rounded-2xl bg-foreground/[0.03] backdrop-blur-sm overflow-hidden divide-y divide-border/20">
          {moodCards.map((card) => {
            ...
            return (
              <button key={card.type} onClick={() => navigate(`/kannetecken/${card.slug}`)}
                className={cn("w-full flex items-center gap-4 px-4 py-5 text-left transition-all duration-200", "hover:bg-foreground/[0.04] active:bg-foreground/[0.06] group", isActive && "bg-foreground/[0.02]")}>
        ...
        <div className="mt-12"><CharacteristicsSharingSection /></div>
      </div>
    </div>
  );
};

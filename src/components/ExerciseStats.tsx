...
export interface ExerciseStatsType {
  exercised: number;
  notExercised: number;
  unregistered: number;
  total: number;
...
export function ExerciseStats({ stats, periodLabel }: ExerciseStatsProps) {
  ...
  const exerciseRate = stats.total > 0 
    ? Math.round((stats.exercised / stats.total) * 100) 
    : 0;

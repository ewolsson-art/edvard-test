                ...
                <div key={weekNumber} className="grid grid-cols-8 gap-0.5">
                  ...
                  {days.map(day => {
                    ...
                    return (
                      <div
                        key={dateStr}
                        title={isCurrentMonth ? `${format(day, 'd MMMM', { locale: sv })}${hasMedication ? ' - Medicin tagen' : ''}` : ''}
                        className={cn(
                          "w-full aspect-square rounded-sm flex items-center justify-center text-[9px] font-medium",
                          !isCurrentMonth && "opacity-0",
                          isCurrentMonth && !hasMedication && "bg-muted/80 text-muted-foreground",
                          isCurrentMonth && hasMedication && "bg-mood-stable text-white",
